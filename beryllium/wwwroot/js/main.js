...
; (function () {
    ...
    'use strict';
    let skip = 3;
    let newsCount = $("#newsCount").val();
    $(document).on("click", "#load-more-news", function (e) {
        e.preventDefault();
        $.ajax ({
            url: "/News/LoadNews?skip="+skip,
            type: "get",
            success: function (res) {
                ...
                res.forEach(elem => {
                    ...
                    <li>
                        <a href="#">
                            <span class="post-date">${elem.time} </span>
                            <h3>${elem.title} </h3>
                            <p>${elem.text} </p>
                        </a>
                    </li> `
    ...
    $(document).on("keyup", ".searchProject", function () {
        ...
        let query = $(".searchProject").val();
        if (query.length > 1) {
            $.ajax({
                url: "/Project/SearchProject?query=" + query,
                type: "get",
                success: function (res) {
                    $(".project-wrapper").append(res);
                }
            })
        }
    })
